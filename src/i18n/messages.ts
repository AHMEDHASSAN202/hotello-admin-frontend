 ...
 *
 * AC 7.1-3: in production a missing key falls back to English rather than
 * showing a raw key path — so non-`en` locales are layered over the full
 * English bundle. In development we return the locale's own bundle untouched,
 * so a missing key surfaces loudly (next-intl logs it and renders the key).
 */
