...
/** One-year cookie so the choice survives sessions on this browser (AC 7.2-2). */
 ...
 * and active filters survive the switch (AC 7.2-3).
 */
export function useSetLocale() {
  ...
  function setLocale(
    ...
    // Persist to the admin profile so the choice follows them across devices
    // (AC 7.2-2). Fire-and-forget — the cookie already covers this browser.
    if (persist) {
      api('/auth/profile', {
        method: 'PATCH',
